...
const EditSaloonDetails = ({ navigation }) => {
    ...
    const getToken = async () => {
        ...
        setCity(newData?.country)
        setZipcode(newData?.zipcode)
        setAvailableDays(newData?.availableDays)
        setOpenTime(newData?.openTime);
        setLunchTime(newData?.lunchTime);
        setCloseTime(newData?.closeTime);
        setSeatCount(newData?.seats);
        setBarberCounts(newData?.barbers)
